...
public class Persons {
    ...
    public String updateGalleryByID(@PathVariable Long id, @RequestBody Person personToUpdateWith) {
        return persons.findById(id).map(foundPerson -> {
            ...
            if (personToUpdateWith.getBirthYear() != 0) foundPerson.setBirthYear(personToUpdateWith.getBirthYear()); //TODO 1check for int/Integer compatibility
            if (personToUpdateWith.getDeathYear() != 0) foundPerson.setDeathYear(personToUpdateWith.getDeathYear()); //TODO 2check for int/Integer compatibility
